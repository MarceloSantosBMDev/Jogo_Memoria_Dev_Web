class Leaderboard {
    ...
    updateMetricHeader() {
        ...
        header.textContent = this.currentMode === 'classico' ? 'Movimentos' : 'Tempo';
    }

    async loadLeaderboard() {
        const leaderboardList = document.getElementById('leaderboard-list');
        leaderboardList.innerHTML = '<div class="loading">Carregando rankings...</div>';
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                leaderboardList.innerHTML = '<div class="no-data">Erro ao carregar rankings.</div>';
            }
        } catch (error) {
    ...
    displayLeaderboard(leaderboard) {
        ...
        if (leaderboard.length === 0) {
            leaderboardList.innerHTML = '<div class="leaderboard-empty">Nenhum jogador ranqueado ainda.<br>Seja o primeiro a jogar!</div>';
            return;
        }
        ...
        leaderboard.forEach((entry, index) => {
            ...
            const metricValue = this.currentMode === 'classico' 
                ? entry.best_score
